...
def reflow_lines(s, depth):
    ...
    column as the first character beyond the first '=' in the first
    line.
    """
    ...
    while len(cur) > size:
        i = cur.rfind(' ', 0, size)
        if i == -1:
            # impossible to reflow line; just append it as is
        ...
        if len(lines) == 1:
            # pad by assignment or parenthesis
            j = cur.find('=', 0, i)
            if j >= 0:
                j += 2  # account for the equals and the space after it
                size -= j
                padding = " " * j
            else:
                j = cur.find('(', 0, i)
...
class StructVisitor(EmitVisitor):
    ...
    def emit_struct(self, fields, cons, name, add_visit, depth):
        ...
        if add_visit:
            self.emit("virtual std::any visit(BaseVisitor&) = 0;", depth + 1)
        self.emit("};", depth)
        self.emit("", depth)
...
class InheritanceVisitor(EmitVisitor):
    ...
    def visitConstructor(self, cons, attrs, sname, depth):
        ...
        self.emit("}", depth + 1)
        self.emit("", depth)
        self.emit("std::any visit(BaseVisitor& v) override {", depth + 1)
